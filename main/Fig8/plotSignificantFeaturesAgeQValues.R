...
OUT_DIR     <- here("scratch", "main", "Fig8")
...
age_by_patient <- unique(clinical[, .(PatientID, Age)])
age_by_patient <- age_by_patient[, .(Age = Age[which.max(!is.na(Age))]), by = PatientID]
...
# ----------------------------
# 3) Morphology per patient+cluster: mean cell area + axis ratio
# ----------------------------
...
# ----------------------------
# 6) Interaction normalisation using cellAdjacency (Epi–Epi, Epi–TME, TME–TME)
# ----------------------------
...
merge_int_norm <- function(count_dt, int_dt, out_col) {
  ...
  agg[, (out_col) := fifelse(aggregated_counts > 0,
                             aggregated_interactions / aggregated_counts, 0)]
...
# ----------------------------
# Uses FNN if available; else brute-force (slower).
...
#adjacency as undirected edges
edges_u <- edges[, .(ImageID, a = FromCellID, b = ToCellID)]
edges_u <- unique(edges_u)
...
if ("TissueStructure" %in% names(ctx)) {
  ...
  #per image, per structure connected components among epithelial cells
...
if (nrow(ductlob_dt) > 0) {
  ...
  ductlob_counts[, Density := N * 1e6 / totalTissueArea]  # match your script
  ...
  warning("Could not compute duct/lobule structures from public adjacency + TissueStructure; skipping these features.")
  ductlob_feat <- data.table(PatientID = unique(cellsclin$PatientID), Age = unique(cellsclin$Age)[1])
}

# ----------------------------
# 10) Compartment summary (patient-level) + rename like legacy
# ----------------------------
...
#rename in case you want to print labels
rename_comp <- c(
...
# ----------------------------
# Combine ALL features into combined_dt (PatientID+Age)
# ----------------------------
...
compute_myoepi_patient_dt <- function(
  ...
  process_one_image <- function(image_id) {
    ...
    #thickness map (pixels) *2 since distmap gives half-thickness
  ...
  img_res <- if (.Platform$OS.type == "unix") {
    ...
    parallel::clusterExport(
      cl,
      varlist = c("epi","MASKDIR","read_myo_mask","process_one_image"),
      envir = environment()
    )
...
# --- merge into combined_dt (per-patient table) ---
combined_dt <- merge(
  combined_dt,
...
p_wide <- reshape.DEswan(res, parameter = 1, factor = "qt")
q_wide <- q.DEswan(p_wide, method = "BH")
setDT(q_wide)

q_cutoffs <- c(0.05, 0.01, 0.005, 0.001)
plot_data_cutoffs <- rbindlist(lapply(q_cutoffs, function(cutoff) {
  numeric_cols <- setdiff(names(q_wide), "variable")
  counts <- q_wide[, lapply(.SD, function(x) sum(x < cutoff, na.rm = TRUE)), .SDcols = numeric_cols]
  v <- as.numeric(counts[1, ])
  names(v) <- gsub("^X", "", names(counts))
  data.table(
    Age_Window_Center    = as.numeric(names(v)),
    Significant_Features = v,
    Q_Cutoff             = factor(cutoff)
  )
}), fill = TRUE)

p <- ggplot(plot_data_cutoffs, aes(x = Age_Window_Center, y = Significant_Features, color = Q_Cutoff)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  theme_classic() +
  labs(subtitle = "Bucket size = 10 years",
       x = "Age",
       y = "No. of significant features",
       color = "q-value\ncutoff") +
  scale_x_continuous(expand = c(0.02, 0)) +
  scale_y_continuous(expand = c(0, 0), breaks = c(0, 70), limits = c(0,75)) +
  scale_color_manual(values = c("0.05" = "grey60",
                                "0.01" = "grey40",
                                "0.005" = "grey20",
                                "0.001" = "black")) +
  theme(
...
out_pdf <- file.path(OUT_DIR, "significantFeaturesAgeQValues.pdf")
ggsave(p, filename = out_pdf, units = "in", width = 6.75, height = 6.25)
cat("Wrote: ", out_pdf, "\n", sep = "")
