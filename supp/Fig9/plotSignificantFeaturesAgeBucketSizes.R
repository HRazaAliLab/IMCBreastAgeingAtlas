...
for (bsize in bucket_sizes) {
  ...
  p_wide_b <- reshape.DEswan(res_b, parameter = 1, factor = "qt")
  q_wide_b <- q.DEswan(p_wide_b, method = "BH")
  setDT(q_wide_b)
  
  signif_counts <- q_wide_b[, lapply(.SD, function(x) sum(x < alpha, na.rm = TRUE)), .SDcols = -1]
  v <- as.numeric(signif_counts[1, ])
  names(v) <- gsub("^X", "", names(signif_counts))
  
  plot_data_list[[as.character(bsize)]] <- data.table(
    Age_Window_Center    = as.numeric(names(v)),
    Significant_Features = v,
    Bucket_Size          = factor(bsize, levels = bucket_sizes)
  )
}

plot_data_combined <- rbindlist(plot_data_list)

p_buckets <- ggplot(plot_data_combined, aes(x = Age_Window_Center, y = Significant_Features, color = Bucket_Size)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  theme_classic() +
  labs(
  ...
  scale_y_continuous(expand = c(0, 0), limits = c(0,135), breaks = c(0,125)) +
  scale_color_manual(
    limits = c("20", "15", "10", "5"),
    values = c("5" = "grey60", "10" = "grey40", "15" = "grey20", "20" = "black")
  ) +
  theme(
...
out_buckets <- file.path(OUT_DIR, "significantFeaturesAgeBucketSizes.pdf")
ggsave(p_buckets, filename = out_buckets, units = "in", width = 4.5, height = 3)
cat("Wrote: ", out_buckets, "\n", sep = "")
